...
import com.meroot.security.model.TransactionDetailsDto;
...
public class PortfolioQueryServiceNoSql implements PortfolioQueryService {
	...
	public PortfolioPositionsDto getPortfolioPositions() {
		...
		if(!portfolioRepository.existsByUsername(AuthenticationUtil.getUsername())) {
			this.portfolioRepository.save(new Portfolio(AuthenticationUtil.getUsername(), Collections.EMPTY_LIST));
		}
	...
	public ListTransactionsDto getPortfolioTransactions() {
		Portfolio portfolio = this.portfolioRepository.findByUsername(AuthenticationUtil.getUsername());
		List<Transaction> transactions = portfolio.getTransactions();
		return createListTransactionsResponse(AuthenticationUtil.getUsername(), transactions);
	}
